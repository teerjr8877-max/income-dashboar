...
export function Sidebar({ currentPage, onNavigate, onResetDemoData, onExportData, saveState, lastSavedAt }) {
  const saveLabel = saveState === 'saving' ? 'Saving…' : saveState === 'error' ? 'Local save issue' : formatSavedTimestamp(lastSavedAt)
    ...
    <>
      <aside className="flex w-full flex-col border-b border-slate-800 bg-slate-950/90 px-6 py-8 lg:min-h-screen lg:max-w-xs lg:border-b-0 lg:border-r">
        ...
        <div className="mt-6 flex items-center gap-3 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-4 py-3 text-sm text-slate-200">
          <span className=${`inline-flex h-2.5 w-2.5 rounded-full ${saveState === 'saving' ? 'bg-amber-300' : saveState === 'error' ? 'bg-rose-300' : 'bg-emerald-300'}`} />
          <span className="font-medium text-white">${saveLabel}</span>
        </div>

        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
          <button type="button" onClick=${onExportData} className="rounded-2xl border border-slate-700 bg-slate-900/60 px-4 py-3 text-sm font-medium text-slate-100 transition hover:border-slate-500 hover:text-white">Export data</button>
          <button type="button" onClick=${onResetDemoData} className="rounded-2xl border border-rose-400/30 bg-rose-500/10 px-4 py-3 text-sm font-medium text-rose-100 transition hover:border-rose-300 hover:text-white">Reset demo data</button>
        </div>
        ...
        <div className="mt-8 rounded-3xl border border-brand-400/20 bg-brand-500/10 p-5 text-sm text-slate-200 lg:mt-auto">
          ...
          <p className="mt-2 text-slate-300">Monitor net worth, contributions, cash flow, and household goals with autosave, local persistence, and one-tap exports.</p>
        </div>
      </aside>
    </>
  `
