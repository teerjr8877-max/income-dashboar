...
export function AccountsPage({ accounts, setAccounts }) {
  ...
  const addAccount = (event) => {
    ...
    const nextAccount = { id: crypto.randomUUID(), ownerId: accountForm.ownerId, accountName: accountForm.accountName.trim(), institution: accountForm.institution.trim(), accountType: accountForm.accountType, balance: Number(accountForm.balance), monthlyContribution: Number(accountForm.monthlyContribution), notes: accountForm.notes.trim(), holdings: [], updatedAt: now }
    setAccounts((current) => [...current, nextAccount], 'Added household account')
  ...
  const submitHolding = (event) => {
    ...
    const now = new Date().toISOString()
    const parsedHolding = { id: editingHolding?.id ?? crypto.randomUUID(), ticker: holdingForm.ticker.trim().toUpperCase(), holdingName: holdingForm.holdingName.trim(), shares: Number(holdingForm.shares), marketValue: Number(holdingForm.marketValue), annualYieldPercent: Number(holdingForm.annualYieldPercent), estimatedMonthlyIncome: calculateHoldingMonthlyIncome(holdingForm), category: holdingForm.category, updatedAt: now }

    setAccounts((current) => current.map((account) => {
      if (account.id !== selectedAccount.id) return account
      const holdings = editingHolding ? account.holdings.map((holding) => (holding.id === editingHolding.id ? parsedHolding : holding)) : [...account.holdings, parsedHolding]
      const balance = holdings.reduce((sum, holding) => sum + Number(holding.marketValue), 0)
      return { ...account, holdings, balance, updatedAt: now }
    }), editingHolding ? 'Updated holding' : 'Added holding')
  ...
  const startEditHolding = (holding) => {
    ...
    setHoldingForm({ ticker: holding.ticker, holdingName: holding.holdingName, shares: String(holding.shares), marketValue: String(holding.marketValue), annualYieldPercent: String(holding.annualYieldPercent), category: holding.category })
  }
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-3 max-w-3xl text-slate-400">Manage each owner’s balance sheet with complete account records, realistic holdings, cloud-backed autosave, and owner-preserving attribution.</p>
      </div>
        ...
        <Panel>
          ...
          <form className="mt-6 space-y-4" onSubmit={addAccount}>
            <div className="grid gap-4 md:grid-cols-2">
              <Select label="Owner" value={accountForm.ownerId} options={ownerOptions} onChange={(value) => setAccountForm((current) => ({ ...current, ownerId: value }))} />
          ...
          <div className="mt-8 space-y-3">
            {accounts.map((account) => {
              ...
              return (
                <button key={account.id} type="button" onClick={() => setSelectedAccountId(account.id)} className={`w-full rounded-2xl border px-4 py-4 text-left transition ${active ? 'border-brand-400 bg-brand-500/10' : 'border-slate-800 bg-slate-950/60 hover:border-slate-700'}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="font-medium text-white">{account.accountName}</p>
                      <p className="mt-1 text-sm text-slate-400">{account.ownerId} · {account.institution} · {account.accountType}</p>
                    </div>
        ...
        <Panel>
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">{selectedAccount ? `${selectedAccount.ownerId} · ${selectedAccount.institution} · ${selectedAccount.accountType}` : 'Add or select an account to manage holdings.'}</p>
            </div>
            {selectedAccount && <span className="rounded-full bg-slate-800 px-3 py-1 text-xs uppercase tracking-[0.2em] text-slate-300">{selectedAccount.holdings.length} holdings</span>}
          </div>
          ...
          <form className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3" onSubmit={submitHolding}>
            ...
            <Input label="Annual yield percent" type="number" step="0.01" value={holdingForm.annualYieldPercent} onChange={(value) => setHoldingForm((current) => ({ ...current, annualYieldPercent: value }))} />
              ...
              <tbody className="divide-y divide-slate-800 bg-slate-900/40">
                {selectedAccount?.holdings.map((holding) => (
                  <tr key={holding.id}>
                    ...
                    <td className="px-4 py-4 text-slate-300">{formatCurrencyPrecise(holding.marketValue)}</td>
                    ...
                    <td className="px-4 py-4"><button type="button" onClick={() => startEditHolding(holding)} className="rounded-xl border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 transition hover:border-brand-400 hover:text-white">Edit</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function Input({ label, onChange, type = 'text', step, value }) {
  return (
    <label className="block space-y-2 text-sm text-slate-300">
      <span>{label}</span>
      <input type={type} step={step} value={value} onChange={(event) => onChange(event.target.value)} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none transition focus:border-brand-400" required />
    </label>
  )
}
...
function TextArea({ label, onChange, value }) {
  return (
    <label className="block space-y-2 text-sm text-slate-300">
      <span>{label}</span>
      <textarea value={value} onChange={(event) => onChange(event.target.value)} rows={4} className="w-full rounded-2xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none transition focus:border-brand-400" />
    </label>
  )
}
